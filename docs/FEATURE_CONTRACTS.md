...
- 效期與清售風險有獨立同步入口，取得全部 FBA 品項與同份報表近 7／30／60／90 天出貨量，接入 FBA 入庫計畫的申報效期；不受 180 天庫齡篩選或舊健檢工作成敗限制。庫齡與官方費用放在補充資料。報表的 `days-of-supply` 與 `snapshot-date` 分別代表目前庫存供應天數及庫存／銷量資料日期；含在途供應天數與庫齡快照日期不是同義欄，可並存但不可替代缺漏的主要證據。真正重複欄及身分歧義仍拒絕。同 SKU 多效期與來源分開保存。歷史申報數量、庫齡桶、SKU 總庫存皆不是現存批次餘量。
