...
- `execute` 以 owner token 原子保留票證；文案、圖片與文案批次另以 owner token 鎖定 listing-attribute collision。多意圖／批次先核對全部 durable availability，必要的整批 revalidation 也在 native approval 前完成；每次 execute 只要求一次 native approval，核准後立即再核對 context 與票證 fence，才允許正式執行。單筆在 approval 後才建立 ledger claim，多意圖／批次正式執行時仍逐 intent 建立 claim
...
- B2B audit owner 與 snapshot 仍固定唯讀；商品列的「設定／調整」只會另行啟動既有單 SKU fresh read／Validation Preview／native approval／單次 PATCH／readback 流程，預設 price-only 完整保留既有 QDP，combined 必須由使用者明確選擇
