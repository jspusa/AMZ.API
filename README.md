...
| 產品 | 整組拖入圖片，依檔名 01–10 數字排序、自動準備公開圖片、格式／像素檢查與 Amazon 回查 | 首次使用下載頁密碼連接圖片服務；實際可用位置依商品規格，Amazon 更新仍需預檢與原生確認 |
...
把 JPEG／PNG 整組拖進圖片工作台，App 會自動準備 Amazon 可讀取的公開圖片。首次使用時，在本機圖片登入視窗輸入下載頁密碼即可繼續；不必另設 R2 或逐張提供網址。檔名可用「品號_01.jpg」或「品號_01_說明.jpg」，依數字排序至第 10 張，實際可編輯位置仍依 Amazon 商品規格。準備中斷會保留原檔，重新準備後再安全預檢與確認送出。

已有 CDN 圖片時仍可直接貼 HTTPS URL，也可在「Notebook Key 安全連線」選配自己的 R2。R2 Secret 只存在作業系統安全儲存區，圖片服務 session 則只保留於 main 記憶體。
