...
- Excel 的 1–5 個非空產品要點對 exact marketplace／language tag 是完整 requested set；fresh raw attributes 即使已有 6–10 個同語系賣點，也以一次 attribute replacement 清除多出的舊值並保留其他語系。adapter 必須從同一份 raw evidence 完整揭露第 6 項後每個將刪原值；揭露納入 batch fingerprint，fresh revalidation 必須逐字相同，renderer 亦以 `pre-wrap` 保留 LF／Tab／CR／U+0085／U+2028／U+2029／連續空白。C0／DEL、其他 C1、零寬、bidi／isolate 與其他不可安全顯示控制符在 main 與 renderer mirror 都拒絕，避免隱藏或重排刪除內容。renderer commit body 還必須回傳完全相同且有序的 affected SKU acknowledgement，缺少、增加、重排或漂移一律在 native approval／PATCH 前以零寫入停止；原生確認摘要必須在 120 字 biometric prompt 內完整列出全部 affected SKU 與 `10→5，刪5` 等範圍，`INVALID` override 亦須完整列出全部 SKU／code。任一高風險摘要無法完整容納就以 `NATIVE_APPROVAL_SUMMARY_TOO_LONG` 在 Write Gate 前停止；多列要求拆批，單列無法再拆時明示改由 Seller Central 人工處理。新版 renderer 若收到缺少 disclosure 欄位的舊 Bridge DTO，必須明示更新 Notebook Key，不能降級成 `null`。普通單 SKU W06 public DTO 不暴露此 batch-only evidence。任何缺 language tag、無法逐字安全顯示的 raw text 或 selector 歧義仍以 `CONTENT_SELECTOR_UNSAFE` 停止，不得猜測
