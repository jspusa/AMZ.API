...
- 新一輪使用者核准的商用首頁色系、資訊層級與動畫重設見 [介面改版紀錄](releases/2026-09-commercial-frontend.md)。頁內導覽必須阻止預設 hash navigation，否則違反 exact renderer document trust。四個新主題樣式位於 composition 最後；仍須保持 fingerprint 與 build rule stream 一致。
