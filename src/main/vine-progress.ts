...
import { parseVineImport, validVineEnrollment, vineEnrollmentKey, VineInputError, type VineRecord, type VineSnapshot } from "../shared/vine";
...
type Persisted = { schemaVersion: 2; profiles: Record<string, VineRecord[]> };
...
export class VineProgressOwner {
  ...
  // Loading, merging, saving and publishing one local view is a transaction.
  // Keep this queue across clear(): an older disk read/write must settle through
  // its context fence before a new operation can rebuild the cache.
  private serial<T>(operation: () => Promise<T>): Promise<T> {
    const work = this.storageQueue.catch(() => undefined).then(operation);
  ...
  private async load(context: SpExecutionContext, revision: number): Promise<void> {
    ...
    const parsed = value === null ? { schemaVersion: 2 as const, profiles: {} } : this.parsePersisted(value, today);
    if (!parsed) throw new SpApiError("本機 Vine 資料格式無法確認，原檔已保留。", { status: 409, code: "VINE_STORAGE_INVALID" });
    if (!this.data) this.data = parsed;
    this.storageUncertain = false;
  }
  private parsePersisted(value: unknown, today: string): Persisted | null {
    if (!value || typeof value !== "object" || Array.isArray(value)) return null;
    const data = value as { schemaVersion: number; profiles: Record<string, unknown[]> };
    if (Object.keys(value).sort().join(",") !== "profiles,schemaVersion" || ![1, 2].includes(data.schemaVersion) || !data.profiles || typeof data.profiles !== "object" || Array.isArray(data.profiles) || Object.keys(data.profiles).length > 20) return null;
    const parsed: Persisted = { schemaVersion: 2, profiles: {} };
    const legacy = data.schemaVersion === 1;
    for (const [key, rows] of Object.entries(data.profiles)) {
      if (!/^[a-f0-9]{64}$/u.test(key) || !Array.isArray(rows) || rows.length > 1_000) return null;
      const enrollments = new Map<string, VineRecord>();
      const legacyKeys = new Set<string>();
      for (const raw of rows) {
        if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
        const fields = legacy ? "asin,claimed,enrolled,enrollmentDate,importedAt,reviews,sellerSku" : "asin,claimed,enrolled,enrollmentDate,importedAt,reviews,sellerSku,status,statusText,title";
        if (Object.keys(raw).sort().join(",") !== fields) return null;
        // v1 did not capture source status. Keep that evidence privately, but
        // never assume those enrollments are active or ended when upgrading.
        const row = (legacy ? { ...raw, title: null, status: "unknown", statusText: null } : raw) as VineRecord;
        if (!validVineEnrollment(row, today) || (legacy && row.sellerSku === null) ||
          ...
          Date.parse(row.importedAt) > this.now() + 1_000) return null;
        const identity = keyOf(row);
        const previous = enrollments.get(identity);
        if (!legacy && previous) return null;
        if (legacy) {
          const legacyKey = JSON.stringify([row.sellerSku, row.asin, row.enrollmentDate]);
          if (legacyKeys.has(legacyKey)) return null;
          legacyKeys.add(legacyKey);
        }
        // Multiple old SKUs can refer to the same ASIN enrollment. Keep its
        // latest observed evidence; fresh page evidence replaces that identity.
        if (!previous || previous.importedAt <= row.importedAt) enrollments.set(identity, row);
      }
      parsed.profiles[key] = [...enrollments.values()];
    }
    return parsed;
  }
  private snapshot(context: SpExecutionContext): VineSnapshot {
    const asOfDate = marketplaceCalendar(US).dayAt(new Date(this.now()));
    const all = this.data!.profiles[this.scope(context)] ?? [];
    return {
      schemaVersion: 2, marketplaceId: US, source: "seller-central-manual", storage: this.storage,
      storageNotice: this.storage === "encrypted-local" ? "只保存在這台 Notebook Key 的加密檔案。" : "加密儲存目前不可用；資料只保留此工作階段，關閉、鎖定或切換帳號後需重新匯入。",
      asOfDate, unconfirmedCount: all.filter((row) => row.status === "unknown").length,
      rows: structuredClone(all.filter((row) => row.status === "active").sort((a, b) => b.enrollmentDate.localeCompare(a.enrollmentDate) || a.asin.localeCompare(b.asin))),
  ...
  async import(request: ApiRequest): Promise<ApiResponse> {
    ...
    if (!body || Object.keys(body).length !== 1 || typeof body.text !== "string" || Object.keys(request.query).length) return invalid("請貼上 Seller Central Vine 頁面內容。");
    ...
    try {
      ...
      return await this.serial(async () => {
        ...
        const existing = new Set((this.data!.profiles[this.scope(context)] ?? []).map(keyOf));
        const knownTerminal = (value: (typeof parsed.rows)[number]["value"]) => value.status === "ended" && existing.has(keyOf(value));
        // Previously verified enrollments may end after leaving the current FBA
        // catalog. Their explicit terminal evidence may retire the local card.
        const needsCatalog = parsed.rows.some(({ value }) => !knownTerminal(value));
        const fba = needsCatalog ? await waitForPromiseWithSignal(this.dependencies.fba(context, signal), signal) : [];
        await this.checkpoint(context, revision);
        throwIfAborted(signal);
        const identities = new Map(fba.map((item) => [item.sellerSku, item.asin]));
        const asins = new Set(fba.map((item) => item.asin));
        if (fba.length > 5_000 || identities.size !== fba.length || fba.some((item) => typeof item.sellerSku !== "string" || !item.sellerSku || !/^[A-Z0-9]{10}$/u.test(item.asin))) throw new SpApiError("FBA 身分資料無法唯一核對。", { status: 422, code: "VINE_FBA_INVALID" });
        const accepted = parsed.rows.filter(({ value, line }) => {
          if (knownTerminal(value) || (value.sellerSku === null ? asins.has(value.asin) : identities.get(value.sellerSku) === value.asin)) return true;
          parsed.rejected.push({ line, message: "目前 US FBA 清單找不到相符的 ASIN（含 SKU 時須完全相同）；此列未匯入。" });
          return false;
        });
        if (accepted.length) {
