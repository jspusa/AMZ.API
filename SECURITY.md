...
- Excel 批次文案寫入在第一次 PATCH 前先鎖定全批 SKU、檢查每 SKU idempotency ledger、重新讀取並完成全批 Amazon Validation Preview。一般 hard preflight failure 仍整批零寫入，且 public response／renderer 必須指出 exact SKU、欄位 diff 與已清理原因。唯一可要求另行確認的情況是 well-formed live Amazon receipt 對 exact SKU 回傳 `INVALID` 且至少含一項 sanitized public `ERROR`；此時狀態只能是 `REQUIRES_VALIDATION_OVERRIDE`，不得冒充預檢通過。renderer 必須讓使用者明確確認 main-owned exact override SKU set；main 在 native approval 前重新取得同一 SKU 的 exact identity／diff／receipt evidence，只有完全相同才可顯示一次列出 override SKU／code 的 Touch ID／Windows Hello 摘要。核准後每 SKU 仍只允許一次 ledgered PATCH attempt 與 canonical readback，Amazon 仍可能拒絕；known rejection 或 unknown 立即停止後續。工作簿篡改、完整讀取失敗、identity／FBA／PTD／schema／context／account／marketplace／mode drift、auth、rate limit、`5xx`、timeout、malformed／unknown receipt 或 post-write uncertainty 一律不可 override，且不得盲目重送
- Excel 的 1–5 個非空產品要點對 exact marketplace／language tag 是完整 requested set；fresh raw attributes 即使已有 6–10 個同語系賣點，也以一次 attribute replacement 清除多出的舊值並保留其他語系。任何缺 language tag 或 selector 歧義仍以 `CONTENT_SELECTOR_UNSAFE` 停止，不得猜測
