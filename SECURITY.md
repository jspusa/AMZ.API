...
- 全站文案／圖片／A+／訂閱／B2B 價格健檢只接收 main process 證明的 FBA SKU；A+ 功能只用 exact marketplace＋ASIN 的公開 publish records 判斷是否有已發布 A+，不提供其他公開 API 無法證明的內容類型功能。任一 exact publish record 優先保留正向證據；warning-only 空清單仍保持未知並引導至固定白名單的 A+ Content Manager，不得冒充未發布。訂閱的 exact-SKU offer／月度問題列只有帶同次 `CURRENT_FBA` 證據才可顯示／匯出，未證明識別值只保留聚合計數；B2B 缺列、身分衝突、offer／QDP 歧義或 seller-specific PTD 失敗必須列為需到 Amazon 後台編輯／資料未完成，不得當成未設定。問題列只能單列隔離並將範圍降為 partial，站點、program、FBA fulfillment、月份或分頁衝突仍整次 fail closed；訂閱與綜合健檢 Excel 只能由 main 保存的短效快照建立
...
- 一鍵全部健檢與單項文案、圖片、A+、未綁變體、訂閱價格、B2B 價格、廣告覆蓋、庫齡、評論都由 main process 綁定 account scope、mode、marketplace 與短效 context；關閉 renderer 抽屜只停止該 observer，不得取消主程序工作，首頁可用 fenced job／context ID 重新接回單調進度與結果。七個 run-all section 的名稱與順序由同一 schema v3 合約固定且各自 fail honest；180 天以上庫齡與評論依此順序放在首頁預設折疊的獨立低頻區，不得偷偷併入 run-all。renderer 不能上傳整份 snapshot 或 account scope，Excel 只由同一 main job 的已驗證結果建立；帳號、模式、站點或 credential lifecycle 改變時舊工作必須取消或失效
