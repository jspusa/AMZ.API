...
- 入庫申報效期優先從已選 placement 的貨件商品取得；無貨件清單才讀計畫商品。每個計畫內按既有批次鍵合併不同貨件的申報量，逐貨件 provenance 留在 main-only checkpoint。同一貨件跨頁重複仍拒絕。個別計畫經驗證的 HTTP 400／404／422 可保留為不可讀並接續其他來源；來源仍不完整，不能推定原因或開放批次提醒。遍歷完成與來源完整分開，終態 partial 不會因 GET 或本機重讀而重送；詳細契約見 [貨件效期來源規格](specs/2026-09-shipment-expiry-source.md)。
