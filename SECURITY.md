...
- Excel 批次文案在建立寫入計畫前仍核對每一列的完整 main-owned digest。只有「已編輯且掃描 readStatus=incomplete」的列可被隔離為 main-owned `blockedChanges`；它必須公開 exact SKU／欄位 diff，明示本次略過且不寫入，並納入批次 fingerprint／500 列上限，但不得成為 write intent、override 或 native approval 對象。其餘可寫列在第一次 PATCH 前仍先鎖定全批 SKU、檢查每 SKU idempotency ledger、重新讀取並完成全批 Amazon Validation Preview。任何可寫列的一般 hard preflight failure 仍整批零寫入，且 public response／renderer 必須指出 exact SKU、欄位 diff 與已清理原因。唯一可要求另行確認的情況是 well-formed live Amazon receipt 對 exact SKU 回傳 `INVALID` 且至少含一項 sanitized public `ERROR`；此時狀態只能是 `REQUIRES_VALIDATION_OVERRIDE`，不得冒充預檢通過。renderer 必須讓使用者明確確認 main-owned exact override SKU set；main 在 native approval 前重新取得同一 SKU 的 exact identity／diff／receipt evidence，只有完全相同才可顯示一次列出 override SKU／code 的 Touch ID／Windows Hello 摘要。核准後每 SKU 仍只允許一次 ledgered PATCH attempt 與 canonical readback，Amazon 仍可能拒絕；known rejection 或 unknown 立即停止後續。工作簿篡改、可寫列的 fresh read 不完整、identity／FBA／PTD／schema／context／account／marketplace／mode drift、auth、rate limit、`5xx`、timeout、malformed／unknown receipt 或 post-write uncertainty 一律不可 override，且不得盲目重送
