        ...
        <figure className={[allZero ? "is-zero" : "", points.length > 30 ? "is-dense" : ""].filter(Boolean).join(" ")}>
          <div ref={plotScrollRef} className="sales-trend-plot-scroll">
            ...
            <desc id={descriptionId}>
              ...
              {snapshot.comparison
                ? `去年同期總銷售 ${formatMoney(snapshot.comparison.totals.totalSales)}。深藍實線為本期，灰色虛線為去年同期。`
                : "目前沒有去年同期比較資料。"}
              可用左右方向鍵逐日查看。
            </desc>
            <defs>
              <linearGradient id={gradientId} x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#2b3a53" stopOpacity="0.18" />
                <stop offset="100%" stopColor="#2b3a53" stopOpacity="0.015" />
              </linearGradient>
            </defs>
            {yTicks.map((tick) => {
