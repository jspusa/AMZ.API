...
## 2026-09-16 全頁工具與圖片進度回查

- 待確認圖片必須顯示同一次回查的具體分類，分開商品／操作證據、圖片位置與 Amazon ERROR；沒有詳細原因時不猜成圖片不符或同步延遲。診斷只含固定分類和有限計數，缺少／畸形或讀取失敗都不沿用舊原因，也不能將 Amazon 圖片網址或相同張數當成回查完成。

- 頂端四個功能群的每個入口直接進入單層完整工作頁；圖片與健檢使用相同寬版呈現，保留所有工具、分頁、返回焦點及寫入忙碌鎖定。此條取代舊條款允許頂端入口沿用 drawer 或只捲至首頁卡片的部分。
- 「重新讀取本批次進度」明確要求一次新的 bounded Amazon GET-only 回查，普通背景輪詢只觀察工作。顯示查詢中、最近回查時間及未相符／失敗原因；只有完整 canonical 圖片目標相符且耐久保存成功才列為已確認。
- 重開或換版後，可用最多 30 個 exact Seller SKU 找回目前帳號、模式、站點的最新圖片操作；較新未知紀錄不得被較舊成功遮蔽。找回及回查不重新準備圖片、不建立預檢或原生批准、不上傳或 PATCH，沒有紀錄不補成成功。
- 價目表隨主畫面載入，不因 Pages 發布後舊延遲檔案消失而永久停在工作區錯誤。保留同次工作與原表內容，實際 Amazon 讀取仍由使用者啟動。

具體範圍與測試見 [本輪規格](specs/2026-09-workspace-image-recovery.md)。
...
- FBA 冗餘庫存只依 Amazon `estimated excess quantity`，庫齡不會被列為冗餘；storage cost／AIS 缺值不會產生假的 0 或部分全站總額。 庫齡區間按整份報表選擇完整且非重疊的一組數量：完整近期細分優先，缺值時才採完整 0–90 彙總；181–365 亦保留標準組優先及完整替代組回退。候選須有完整標題與每列有效數量，不能逐列混用、空白補零或拼接區間；回退前檢查的非空非法值仍拒絕。區域尾桶與費用規則保持獨立。
...
- 效期與清售風險有獨立同步入口，取得全部 FBA 品項與同份報表近 7／30／60／90 天出貨量，接入 FBA 入庫計畫的申報效期；不受 180 天庫齡篩選或舊健檢工作成敗限制。效期流程使用獨立的核心報表投影，不因補充庫齡缺值阻擋有效庫存、銷量及入庫效期整理；健康資料的未知 180 天以上數量為 null，保存及顯示不得補零。原完整庫齡健檢仍嚴格校驗，補充數量及官方費用各自須有完整證據，非空非法值或格式／身分歧義仍拒絕。庫齡與官方費用放在補充資料。報表的 `days-of-supply` 與 `snapshot-date` 分別代表目前庫存供應天數及庫存／銷量資料日期；含在途供應天數與庫齡快照日期不是同義欄，可並存但不可替代缺漏的主要證據。真正重複欄及身分歧義仍拒絕。同 SKU 多效期與來源分開保存。歷史申報數量、庫齡桶、SKU 總庫存皆不是現存批次餘量。
- 入庫申報效期優先從已選 placement 的貨件商品取得；成功明細無貨件清單時讀計畫商品。明細遭精確固定 400／BadRequest／parsed／other-input 拒絕時，可依同輪清單身分獨立讀取計畫申報商品；已存同 revision 的相同失敗不重送 metadata，完整商品來源與未核對的貨件明細分開標示，申報量不當成現存餘量。現行備援與保存界線見 [計畫申報效期備援](specs/2026-09-plan-declared-expiry-fallback.md)。每個計畫內按既有批次鍵合併不同貨件的申報量，逐貨件 provenance 留在 main-only checkpoint。同一貨件跨頁重複仍拒絕。個別計畫經驗證的 HTTP 400／404／422，在不符合備援條件或商品讀取仍失敗時，保留為不可讀並接續其他來源；來源仍不完整，不能推定原因或開放批次提醒。遍歷完成與來源完整分開，終態 partial 不會因 GET 或本機重讀而重送；詳細契約見 [貨件效期來源規格](specs/2026-09-shipment-expiry-source.md)。
- 效期來源摘要只投影 main 驗證的既有 checkpoint 計數、固定失敗操作／頁次／HTTP／原因及紀錄時間；本機 GET 不發 Amazon request。超過 cursor 時限的紀錄可明標歷史摘要，不恢復過期 cursor、不更新資料有效性。舊紀錄未保存的失敗步驟保持未知，摘要不授權批次確認或行事曆。詳細規格見 [來源讀取診斷](specs/2026-09-expiry-source-read-diagnostics.md)。
...
- US 價目表可不放原 Excel，從已確認的 FBA SKU／ASIN 產生含品名、Amazon 售價、最低價設定、首圖與缺值原因的新 `.xlsx`。缺價格不補 0；個別缺圖或 exact `409 LISTING_IDENTITY_MISMATCH` 商品列標為未完成，保留已驗 FBA 身分，不採用被拒絕的 Listing 欄位，其他商品繼續。帳號／模式／context、全批身分、認證及網路錯誤仍停止。原表比對與原檔保留流程保持可用。
...
Supply Boss 圖片只暫存一小時，新準備與確認必須核對有效 expiry；保留超過十分鐘餘裕，核對票證不超過最早來源期限減十分鐘或原十五分鐘上限。既有圖片依可信建立時間縮短期限；缺少建立時間時只有一次固定一小時遷移緩衝。固定每五分鐘維護工作只刪除到期圖片；合法新上傳附帶最多一筆背景清理，不等待清理完成。確認實體不存在且 reservation CAS 成功後才回收容量，未確認仍占額度；網址過期不冒充清理已完成。排程／服務中斷保留失敗證據，不能顯示釋放成功；本機與 OneDrive 原檔不修改。既有單 SKU 工作保留，新分頁缺 Notebook Key 能力時提示更新。
