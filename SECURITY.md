...
- 所有 Amazon mutation 的共用安全機制由 main-owned Write Gate 統一持有；`stagePreview`、`execute`、`reconcile` 與 `clearEphemeral` 不暴露 scoped fingerprint、owner token、native approval 或 ledger internals 給 renderer／preload，router 也只呼叫這個 seam 而不編排 ledger 或 native confirmation。票證精確綁定 context、account scope、operation、proposal fingerprint 與 idempotency key；一般票證有效 2 分鐘，文案批次有效 15 分鐘
- `execute` 以 owner token 原子保留票證；文案、圖片、文案批次與 Standard／Sale／Business offer family 另以 owner token 鎖定 marketplace＋SKU collision。Business Price 的最低價＋B2B 兩個 intent 會把同一個去重後 reservation 持有到整段 session finalizer，不能在兩階段間插入另一筆價格寫入。多意圖／批次先核對全部 durable availability，且 Standard／Sale／Business 共用的 offer-family `pending`／`unknown` collision 會在 native approval 前停止；唯一 duplicate-plan repair 例外在 availability 與正式 claim 使用完全相同的窄條件。必要的整批 revalidation 也在 native approval 前完成；每次 execute 只要求一次 native approval，核准後立即再核對 context 與票證 fence，才允許正式執行。單筆在 approval 後才建立 ledger claim，多意圖／批次正式執行時仍逐 intent 建立 claim
- `LocalStore` 是 durable write evidence owner，狀態固定為 `pending`、`completed` 或 `unknown`。durable fingerprint 維持 account-scoped fingerprint 後再由 LocalStore SHA-256 一次的既有 double-hash，相容既有 ledger 與文案批次衍生 key；pre-commit failure 安全刪除 claim，非 pre-commit 的 `401`／`429`、可能已送出的 request、timeout 或 ambiguous readback 必須留下 `unknown` 並禁止重送
- Credential、lock、suspend 或 account／mode invalidation 會清除短效票證並增加 ephemeral generation，但不刪除 durable `pending`／`unknown`，也不提前釋放仍在執行中的 listing-attribute claim；active claim 只由該次 `execute` 的 finalizer 釋放。`reconcile` 失敗保持 fail closed，只有 exact canonical readback projection 才能完成既有 durable evidence
...
- B2B audit owner 與 snapshot 仍固定唯讀；商品列的「設定／調整」只會另行啟動單 SKU fresh read／Validation Preview／native approval／ledgered PATCH／readback 流程。階梯可安全編輯時介面預設 combined 並明列價格、折扣與最低價差異；使用者仍可切到 price-only，此模式完整保留既有 QDP 且絕不變更最低價
...
- B2B 價格／數量折扣寫入只接受 exact FBA SKU、同幣別標準價／B2B 舊值、完整舊 QDP hash 與 seller-specific PTD checksum；price-only 固定只 replacement exact `audience=B2B` contribution、保留 canonical QDP，且絕不攜帶最低價。使用者明確提交 canonical percent tiers、QDP PTD 能力獨立證明，且 US／USD 最低階單價低於一份 canonical 既有 `ALL.minimum_seller_allowed_price` 時，才可啟動唯一的最低價例外：main-owned opaque descriptor 保存完整原 `ALL` contribution，PATCH 只替換 `minimum_seller_allowed_price` 為最低階單價少整數 100 cents；absent／ambiguous 最低價、非 USD、重複階梯修復、非正值目標或 descriptor 身分／hash 不一致一律 fail closed。正式寫入前必須有最低價單獨 Preview 與 ALL＋B2B final-state Preview 的 exact VALID 證據；一次 Touch ID／Windows Hello 摘要把 ALL old→new 與一般／自動定價影響放在 120 字截斷前。核准後固定最低價 ledgered single PATCH＋canonical readback，再 fresh B2B-only Preview、B2B ledgered single PATCH＋price／tiers canonical readback；整段以 marketplace＋SKU offer reservation 互斥。最低價未確認前不得送 B2B；最低價已完成而後段 known failure 必須回 partial，任一已送出結果不明則保留 unknown 且不得盲目重送
