...
Catalog reports／B2B 唯讀抽離在 runtime 之上另設 `FbaCatalogReports` semantic coordinator。它只在明確的 begin 動作要求 `ReportsRuntime` 建立固定 All Listings；B2B begin 才同時要求固定 Active Listings。status、既有資料讀取與 optional Active evidence lookup 都只能沿用 opaque lease／document handles，不得隱含 create。`ReportsRuntime.readDocument()` 仍是 report metadata、原始 report／document ID、signed URL 與下載的唯一 owner；coordinator 只取得已下載文字並在讀取前後重驗 execution context，不擁有 HTTP transport、credential、store、workbook、PTD、Preview、PATCH 或其他寫入能力。一般 Active unavailable 可以降為「此來源目前不可用」並繼續保留其他 exact positive，但 account／mode／generation fence 與 AbortError 不得被 fallback 吞掉。
...
Brand／Category 與 Sales & Traffic 也在同一 runtime 之上分成兩個深模組。`FbaRevenueReports` 完整擁有品牌營收的 All Listings＋FBA shipment 雙 leg durable job、immutable marketplace cutoff、A→B→A reuse、single-flight、terminal／unknown tombstone、snapshot cache 與 execution-context fence；`ApiRouter` 只保留 request validation、HTTP status 與公開 DTO 轉譯。品牌與八個品類永遠由同一份 shipment document 建立一個 snapshot，current Marketplace Day 保留 job 建立當下的真實 `dataThrough` 並標示 `includes-current-day`，未分類品牌與 `other` 品類規則不變。`SalesAndTrafficReports` 只接受 marketplace 與完整日範圍，內部固定 DAY＋SKU intent 並沿用既有 opaque handles；status／read 不得隱含 create，日期 selection 回到既有相容範圍時沿用 durable lease。`revenue-report-windows.ts` 是唯一的 immutable window／strict report date owner；`brand-sales-reads.ts` 與 `sales-and-traffic-reads.ts` 只解析已下載文字與 canonical FBA catalog seeds，不接 runtime、transport、store、credential、renderer 或 write seam。Sales & Traffic 對 report type／options／marketplace／日期、唯一 SKU／ASIN、非負安全整數／金額與幣別全部 exact fail closed；缺列保持未回報，不補 0。兩個 demo source 都從注入的既有 FBA catalog demo fixture 取得同一組 SKU／ASIN，不能作為 live fallback。舊 Brand／Sales & Traffic create、status、poll、download facade 與 router gateway 已移除，architecture guard 禁止重新引入。

FBA inbound shipment 與 noncompliance 唯讀流程由 main-only `FbaInboundReads` 單一 semantic owner協調；它只公開 `readShipments()`與 `readNoncompliance()`。`ApiRouter` 以 account、mode、marketplace與精確日期區間建立背景 job時只捕捉一份不可變 `SpExecutionContext`，兩條 read leg共用該 context，router不再直接擁有 shipment fallback或 report create／poll／download／parse lifecycle。shipment production adapter只接受封閉的 typed identity並固定 Fulfillment Inbound v0與 2024-03-20 GET endpoints、region-global pacing、timeout、一次 401 refresh與 bounded transient retries；caller不能指定 host、URL、method、body、credential或 retry control。若 v0日期範圍清單明確回傳 400／422，才依序改用固定 v0活動狀態清單與固定 modern plan／shipment GET；403、429、5xx、network、timeout或 abort皆不觸發語意 fallback。備援清單不具完整所選日期證據，因此即使逐貨件 items都讀完，整體工作仍必須是 partial。semantic layer預先取得的第一個 shipment-list page可交給 collector，後續只以同一 opaque token與原 shipment binding呼叫固定 `/fba/inbound/v0/shipmentItems?QueryType=NEXT_TOKEN`；每頁檢查可見 Shipment ID、token前進、重複 SKU、頁數與全域列數，任何衝突只保留已核對列並把該票標為 partial。每日 noncompliance intent仍交給同一 `ReportsRuntime`，由耐久化 broker唯一擁有建立、輪詢、opaque handle與文件下載；semantic layer只在 ready後解析文字。renderer只取得已清理的 shipment／carton／product問題列，不取得內部 plan／shipment ID、report ID、document ID、帳號 scope或任意下載 URL。舊 `sp-api.ts` inbound production facade與 router gateway已移除，architecture guard禁止重新引入。
