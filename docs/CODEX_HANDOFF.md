...
更新：2026-09-09。Repository：`jspusa/AMZ.API`；控制台：`https://jspusa.github.io/AMZ.API/`。
...
- 0.1.61 已由 PR #230／main `cd2616c911e0812bfe3f96957a30b413cd4a0951` 完成四條 exact main CI、兩平台可信 artifact、Mac 正式安裝／連線及兩份 portal 上傳；登入後下載仍待驗證。原始 TPZ preparation 保留唯讀 shape／size／theme，但 2026-09-09 實帳 Preview 仍回 missing Contains Liquid Contents、未帶出輸入欄，不能宣稱修完或 Preview VALID。Issue #228 保持 open；[renderer-only 診斷](specs/2026-09-09-variation-preview-diagnostics.md) 已由 PR #233／main `1e512e8` 發布並驗證全部 Pages bytes，實帳確認 `VALIDATION_FAILED`／`90220`／`contains_liquid_contents`／`MISSING_ATTRIBUTE`，仍未觀察 raw HTTP、PTD 或既有值過濾分支。接續 [main 衝突原因 spec](specs/2026-09-09-variation-required-field-conflicts.md)，先用可還原本機 candidate 辨識實際分支，再修正正式版；不猜缺欄原因，不以 source dependency patch 充當已安裝更新。完整交付與未完成驗收以 [0.1.61 帳本](releases/2026-09-variation-preview-required-fields.md) 為準；最新 renderer-only main `e529ffa` 的變體整合及 Pages bytes 也已另驗證。未執行正式 Amazon mutation。
