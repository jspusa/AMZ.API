...
- B2B 價格／數量折扣寫入只接受 exact FBA SKU、同幣別標準價／B2B 舊值、完整舊 QDP hash 與 seller-specific PTD checksum；price-only 固定只 replacement exact `audience=B2B` contribution、保留 canonical QDP，且絕不攜帶最低價。使用者明確提交 canonical percent tiers、QDP PTD 能力獨立證明，且 US／USD 最低階單價低於一份 canonical 既有 `ALL.minimum_seller_allowed_price` 時，才可啟動唯一的最低價例外：main-owned opaque descriptor 保存完整原 `ALL` contribution，PATCH 只替換 `minimum_seller_allowed_price` 為最低階單價少整數 100 cents；absent／ambiguous 最低價、非 USD、重複階梯修復、非正值目標或 descriptor 身分／hash 不一致一律 fail closed。每個 intent 都只有一次 ledgered PATCH；Amazon 回格式完整的 accepted receipt 後，commit route 不呼叫 canonical GET，而是立刻以 HTTP `202`／`PROCESSING` 保存 accepted time、Request ID、submission ID、完整 target 與 `canResend:false`，不把可能長達數分鐘、實際曾接近 10 分鐘的同步延遲冒充失敗，也不背景輪詢或重送。只有之後由使用者手動重新讀取或其他明確 GET 所取得的 exact canonical SKU／ASIN／marketplace／價格／最低價／QDP 全部相符，`reconcile` 才能將 durable evidence 轉為 `VERIFIED`。最低價寫入與 B2B 寫入固定拆成兩次授權：第一次 fresh Preview 與 Touch ID／Windows Hello 只允許最低價 single PATCH；最低價 `ACCEPTED` 後一定停止並明示 B2B 尚未送出。最低價日後 verified 後，必須以最新 Listing 重新執行 B2B Preview 並取得新一次 Touch ID／Windows Hello，才可建立 B2B claim 與 single PATCH；不能沿用已結束的 approval、在背景續送或把第一次 preview 當成第二次授權。沒有 accepted receipt 的 transport uncertainty 或其他已送出結果不明仍保留 unknown 且不得盲目重送
