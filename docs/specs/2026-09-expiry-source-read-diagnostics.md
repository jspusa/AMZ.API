...
3. 新紀錄只保留固定操作種類（計畫資料、貨件商品、計畫商品）、首頁／後續頁、真實 terminal HTTP 400／404／422 、固定 Amazon 錯誤代碼／回應讀取狀態及 bounded allowlist 原因。來源 ID、Seller／商品資訊、Request ID、URL、headers、原始 body、任意 error message 與 raw checkpoint 不可進入診斷 DTO。
...
6. 不更改 method、path、query、pageSize、chosen-placement 選取或遍歷條件。不新增 Amazon 診斷入口、自動重試或任意 transport。Amazon 官方模型中 `getInboundPlan` 的完整 malformed-ID 訊息及 `listShipmentItems` 的完整 plan-not-exist 訊息，只在各自 operation／400／BadRequest 條件下分類；不得用關鍵字、部分匹配或範例推定 live 原因。舊 checkpoint 未保存的 code／responseState 保持 `not-recorded`，不冒充沒有讀取或未知錯誤。既有一次明確同步會保存新診斷；GET／本機重讀始終零上游請求。
...
- 分開驗證已解析的 BadRequest／InvalidInput、未知回應與空白／格式／過大／逾時／未讀取狀態，保持保存及重開後分類不遺失；非法字串、跨 operation 及近似訊息不可被判成官方原因。
...
固定分類只依 [Amazon 官方 Fulfillment Inbound 2024-03-20 模型](https://github.com/amzn/selling-partner-api-models/blob/main/models/fulfillment-inbound-api-model/fulfillmentInbound_2024-03-20.json) 中對應 operation 的 400 sandbox 範例；它是分類測試依據，並非本帳號 live 證據。
