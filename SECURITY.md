...
- 文案健檢 Excel 只接受 AMZ.API schema v2 `.xlsx`；main process 拒絕公式、巨集、外部連結、Defined Names、未知欄、重複 SKU、異常 ZIP/XML 與超限內容。帳號、站點、live／demo、export ID、時間、SKU／ASIN／Product Type、變體分類與全部原始文案都必須和裝置端 24 小時 SHA-256 列證據精確一致；持久檔不保存 SKU、ASIN、文案、Excel 或 proposed edits，鎖屏／重啟不展延期限。顏色與 rich text 不作授權依據，只採用「更新」欄的實際差異
