...
- B2B audit owner 與 snapshot 仍固定唯讀；商品列的「設定／調整」只會另行啟動單 SKU fresh read／Validation Preview／native approval／ledgered PATCH／readback 流程。階梯可安全編輯時介面預設 combined 並明列價格、折扣與最低價差異；使用者仍可切到 price-only，此模式完整保留既有 QDP 且絕不變更最低價。首頁 audit 與 editor 共用同一個寬版單層 workspace，非首頁入口則沿用同一 presentation surface 內互斥的內容 view；兩者都不建立第二個 modal，processing 狀態、送出時間、Request ID 與 canonical 差異在 editor 內持續可見
