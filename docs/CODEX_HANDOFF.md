...
- 0.1.61 已由 PR #230／main `cd2616c911e0812bfe3f96957a30b413cd4a0951` 完成四條 exact main CI、兩平台可信 artifact、Mac 正式安裝／連線及兩份 portal 上傳；登入後下載仍待驗證。原始 TPZ preparation 保留唯讀 shape／size／theme，但 2026-09-09 實帳 Preview 仍回 missing Contains Liquid Contents、未帶出輸入欄，不能宣稱修完或 Preview VALID。Issue #228 保持 open；[renderer-only 診斷](specs/2026-09-09-variation-preview-diagnostics.md) 已由 PR #233／main `1e512e8` 發布並驗證全部 Pages bytes，實帳確認 `VALIDATION_FAILED`／`90220`／`contains_liquid_contents`／`MISSING_ATTRIBUTE`。2026-09-10 可還原診斷版進一步證明 Amazon HTTP 200 與 `VARIATION_REQUIREMENTS_VALUE_CONFLICT`：來源已有非空事實而被 fill-only 表單排除；未曝光 raw PTD 或假定實際答案。診斷後已還原可信 CI 0.1.61、vault bytes 相同；接續 [保留既有必填事實 spec](specs/2026-09-10-variation-preserved-required-facts.md) 修正，source 0.1.62 已完成實作、兩軸複審及 feature check（296 files／3,316 tests、audit 0），整合／發布仍待完成，不能充當已安裝更新。完整交付與未完成驗收以 [0.1.61 帳本](releases/2026-09-variation-preview-required-fields.md) 為準；先前 renderer-only main `e529ffa` 的變體整合及 Pages bytes 也已另驗證。未執行正式 Amazon mutation。
