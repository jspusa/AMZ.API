...
- GitHub Pages provides the current UI; macOS AMZ.API Key Bridge owns credentials
  and Amazon SP-API network calls.
...
- Preserve the renderer/preload/main trust boundary, FBA filtering, write previews,
  Touch ID confirmation, idempotency ledger, and no-blind-retry policy.
...
- CI cannot prove live Amazon behavior. State clearly what still requires the
  user's Mac and Amazon account.
