...
回查仍待確認時，可展開「查看本次回查原因」，分別查看商品資料、圖片位置與 Amazon 回報的錯誤。不同的 Amazon 圖片網址只能說明目前無法直接對上，不能單憑網址或張數判定完成。
...
| 產品 | 全站 FBA 文案健檢（產品名稱 `<60`、產品亮點 `<110`、每項產品要點 `<150`／`>200`、產品敘述 `<1800`、疑似錯字、缺值、成分宣稱核對與逐項原因） | Amazon 唯讀掃描；第一次按下就啟動一個工作，忙碌期間重複點擊只沿用同一 flight；依明確 ingredients 證據核對多成分、Tendon／Tendons、Chicken／hypoallergenic 及 Grain-free／穀物宣稱；成分宣稱核對不含產品描述；`airdried`、`grainfree`、`dogfood`、`airdry` 只在後台產品敘述免列錯字，出現在標題、亮點、要點或成分仍回報；摘要與常用操作優先顯示，門檻、辭典與安全範圍收在低調的「詳細說明 ›」；Excel 依已證明的變體 family 分頁，問題欄著色，未發現問題的可編輯欄使用淺綠色。只有 family 每一列資料完整且全部檢查通過時，工作表才由 `F001` 命名為 `F001(可)`；任一問題、incomplete 或 unknown 都維持原名 |
...
| 產品 | 在變體工作台從未綁健檢選商品，以同系列已綁品號與星等參考建議 family；有完整原值的唯讀維度可保留並加入 | 建議沿用已確認快照，選定後重新查詢；不因品號相似自動綁定，也不 PATCH 唯讀值 |
...
「圖片／資料夾批次更新」可直接拖入一張或多張 JPEG／PNG。每張圖可選 01–10 位置、從同系列 FBA 建議勾選商品，或每行手動貼上完整 SKU；同一張圖可一次套用多個 SKU。檔名如「AFA21AM_09_系列底圖.jpg」會帶入 AFA21AM 與第 9 張；只更新指定位置，其他圖片由主程式保留。最多 30 個 SKU、300 個套用組合；逐項核對後一次原生確認。此能力需要 Notebook Key 0.1.82，舊版仍可使用原資料夾流程。完整邊界見[共用圖片規格](docs/specs/2026-09-shared-image-batch.md)。

Notebook Key 0.1.83 起顯示圖片準備與逐 SKU 核對進度。同系列建議可點整列選取，並提供全選／取消全選；手動加入的 SKU 保留。

Notebook Key 0.1.84 起，先前圖片已明確由 Amazon 受理時，可另做新的圖片更新，不必等舊圖片網址完全相符；同圖重新套用、換位或還原也需重新核對並通過本次原生確認。舊紀錄仍保留「已受理、尚未確認」，不會冒充本次成功。真正結果不明時則保留新圖草稿並唯讀回查，不能靠重選圖片重送。完整邊界見[新圖片更新規格](docs/specs/2026-09-image-new-update-after-accepted.md)。
...
單 SKU 文案更新在 Validation Preview 後直接跳本機身分確認，不再要求重打 SKU；可保留 1–5 項產品要點。只有產品要點本身有編輯時，main 才能把同語系第 6 項後舊值作為 exact replacement 範圍：App 必須逐項顯示 Amazon 原值、更新值與所有將刪內容，使用者勾選和本次 preview 綁定的確認後，再進 Touch ID／Windows Hello；只改標題或其他欄位時，多出的產品要點保持原樣。Excel 批次文案先核對每列的 main-owned 掃描證據，並一次列出所有問題 SKU、工作表、Excel 列號、實際欄位與清理過的公開原因；能安全歸屬 exact SKU 的 parser／fresh-read／Validation／身分／FBA／PTD／schema 問題只隔離該 SKU 且保持零寫入，其餘獨立安全列繼續。Amazon `INVALID` 永遠隔離，沒有 override 或「仍要嘗試上傳」入口。工作簿結構篡改，或無法安全歸屬單一 SKU 的 context／帳號／站點／模式 drift、auth／rate limit／`5xx`／網路／逾時、malformed／unknown、結果綁定失效等全域問題仍停止整批。Excel 對同語系產品要點採完整取代時，adapter 若看到第 6 項後舊值，必須把所有將刪內容及語系帶進 main-owned 預檢；renderer 逐字顯示並回傳完全相同且有序的 SKU acknowledgement，main fresh revalidation 也必須逐字相同。Preview 與原生確認前重新預檢最多同時處理 3 個 SKU，同一 phase 只為相同 Product Type 與安全脈絡沿用 PTD；原生確認只涵蓋重新預檢後仍可寫的 SKU。Excel 預檢、原生確認與正式送出尚未完成時，外層關閉／返回、工具分頁及 Amazon 站點選擇都會鎖住，避免失去本次 main-owned 進度。摘要超過 120 字時改用不含任意 Amazon 文字的 bounded 計數與 Write Gate 12 字驗證碼，因此大量更新不必拆檔。核准後正式 PATCH 仍按工作簿順序單線送出；每筆 durable `ACCEPTED` 後立刻送下一筆，最多 2 個 main-owned GET-only 工作另外做 bounded canonical reconciliation。`ACCEPTED`、verified 與 unknown 分開顯示；任何回查都不會重送 PATCH。價格與 Sale Price 保留各自既定的額外防呆；圖片保留預檢與原生確認，但不需重打 SKU。
