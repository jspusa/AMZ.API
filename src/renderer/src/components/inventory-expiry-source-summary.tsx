    ...
    {!diagnostic ? <p>目前 Notebook Key 未提供來源摘要；更新後可讀取已保存的同步紀錄。</p>
      : diagnostic.status === "unknown" ? <p>{unknownLabels[diagnostic.reason]}</p>
        : <>
          ...
          <p>{`本輪已列出 ${diagnostic.listedPlanCount} 個計畫；已保存完整讀取 ${diagnostic.cachedPlanCount} 個；無法讀取 ${diagnostic.unavailablePlanCount} 個；待讀取 ${diagnostic.pendingPlanCount} 個。`}</p>
