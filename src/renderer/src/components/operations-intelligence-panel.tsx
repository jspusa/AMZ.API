...
const STATUS_LABELS: Record<OperationsSourceState["status"], string> = { never: "未同步", running: "同步中", complete: "已核對本次範圍", partial: "部分完成", failed: "同步失敗" };
...
export default function OperationsIntelligencePanel({
  ...
  return <section className="operations-intelligence" aria-labelledby="operations-intelligence-title">
    ...
    {state && <div className="oi-source-status" aria-live="polite"><div><strong>{STATUS_LABELS[state.status]}</strong>{stale && <b className="oi-stale">可能過期</b>}{state.status === "failed" && <p>{state.message}</p>}{state.status !== "never" && <details className="oi-details"><summary>時間</summary><small>{state.fetchedAt ?? "尚未取得"}{ageMinutes !== null && ` · ${ageMinutes} 分鐘前`}{state.nextSyncAt && ` · 下次 ${state.nextSyncAt}`}</small></details>}</div></div>}
    <div id={`oi-view-${view}`} className="oi-view" tabIndex={0}>{view === "events" && visible ? <><details className="oi-details"><summary>說明</summary><p>{visible.notice}</p><p>已知悉不等於問題已解決；只有來源完整重新核對後，才可解除事件。</p></details><div className="oi-events">{visible.events.slice(0, eventPage.limit).map((event) => <article className={`oi-event severity-${event.severity}`} key={event.id}><div><span className="oi-event-status">{{ open: "待處理", acknowledged: "已知悉", resolved: "已解除" }[event.status]}</span><h4>{event.title}</h4><p>{event.detail}</p><small>{event.sellerSku ?? "來源範圍"} · {event.lastObservedAt}</small></div><div className="oi-event-actions"><button type="button" aria-label={`查看來源：${OPERATIONS_INTELLIGENCE_SOURCE_LABELS[event.source]}`} onClick={() => selectView(event.source)}>查看來源</button>{event.status !== "resolved" && <button type="button" disabled={busy} onClick={() => void acknowledge(event.id, event.status === "open" ? "acknowledged" : "open")}>{event.status === "open" ? "已知悉" : "恢復"}</button>}</div></article>)}</div>{visible.events.length > 0 && <MoreRows limit={eventPage.limit} total={visible.events.length} onMore={eventPage.more} label="事件" />}{visible.omittedEventCount > 0 && <p>另有 {visible.omittedEventCount} 筆</p>}</> : data ? <>{data.warnings.length > 0 && <ul className="oi-warnings">{data.warnings.map((warning, index) => <li key={index}>{warning}</li>)}</ul>}{view === "promotions" && "promotions" in data && <Promotions data={data} />}{view === "awd" && "stockScope" in data && <Awd data={data} />}{view === "price-health" && !("promotions" in data) && !("stockScope" in data) && !("kind" in data) && <PriceHealth data={data} />}{view === "advertising" && "kind" in data && <Advertising data={data} />}</> : null}</div>
  </section>;
}
